...
def _yolo_bbox_center_to_3d(model, rgb, depth, intrinsics, cam_pos, cam_mat):
    ...
    return _camera_point_to_world(point_cam, cam_pos, cam_mat), (
        x_min,
        y_min,
        x_max,
        y_max,
...
def main() -> None:
    ...
    )  # distinct from generate_dataset's default train/val seed
    ...
    baseline_miss_indices, candidate_miss_indices = set(), set()
    ...
    while evaluated < args.count:
        ...
        if attempts > args.count * 10:
            raise RuntimeError(
                "Too many degenerate samples during evaluation -- check camera/range setup."
            )
        ...
        if baseline_point_cam is None:
            baseline_misses += 1
            baseline_miss_indices.add(evaluated - 1)
            baseline_world = None
        else:
            baseline_world = _camera_point_to_world(
                baseline_point_cam, cam_pos, cam_mat
            )
            baseline_errors.append(float(np.linalg.norm(baseline_world - truth)))

        yolo_result = _yolo_bbox_center_to_3d(
            model, rgb, depth, intrinsics, cam_pos, cam_mat
        )
        if yolo_result is None:
            candidate_misses += 1
            candidate_miss_indices.add(evaluated - 1)
        ...
        if saved < args.save_samples:
            ...
            scaled = cv2.resize(
                bgr, (args.width * 6, args.height * 6), interpolation=cv2.INTER_NEAREST
            )
            ...
            cv2.putText(
                scaled,
                f"base_err={baseline_errors[-1]:.4f}"
            ...
            cv2.putText(
                scaled,
                f"yolo_err={candidate_errors[-1]:.4f}"
    ...
    candidate_mean = (
        float(np.mean(candidate_errors)) if candidate_errors else float("nan")
    )
    candidate_max = (
        float(np.max(candidate_errors)) if candidate_errors else float("nan")
    )

    print(f"\n=== Evaluated {evaluated} frames ===")
    print(
        f"Baseline (RGB threshold): mean_err={baseline_mean:.4f}m max_err={baseline_max:.4f}m misses={baseline_misses}"
    )
    print(
        f"Candidate (YOLO):         mean_err={candidate_mean:.4f}m max_err={candidate_max:.4f}m misses={candidate_misses}"
    )

    if baseline_miss_indices == candidate_miss_indices:
        print(
            "Miss sets: identical (baseline/candidate means are directly comparable)."
        )
    else:
        symmetric_diff = baseline_miss_indices ^ candidate_miss_indices
        print(
            f"Miss sets: DIFFER by {len(symmetric_diff)} frame(s) -- "
            "baseline/candidate means are computed over non-identical frame sets."
        )
    ...
    if baseline_mean > 0 and not np.isnan(candidate_mean):
        ...
        print(
            f"GO/NO-GO (>=40% improvement AND <1cm absolute): {'GO' if go else 'NO-GO'}"
        )
    else:
